"use strict";
let Asdasds = require('./asdasds.model'),
...
const create = (req, res) => {
  ...
  req.getValidationResult().then(function (result) {
    if (!result.isEmpty()) {
      ...
      let asdasds = new Asdasds(data);
      asdasds.save((err, docs) => {
        if (err)
...
const get = (req, res) => {
  Asdasds.findById(req.params.id, (err, docs) => {
    if (err) {
      return sendJSONResponse(res, HTTPStatus.INTERNAL_SERVER_ERROR, {
        success: false,
        message: err
...
const patch = (req, res) => {
  ...
  Asdasds.findByIdAndUpdate(req.params.id, {$set: data}, {new: true}, function (err, docs) {
    if (err) {
      return sendJSONResponse(res, HTTPStatus.INTERNAL_SERVER_ERROR, {
        success: false,
        message: err
...
const remove = (req, res) => {
  ...
  Asdasds.findByIdAndRemove(req.params.id, (err, docs) => {
    if (err) {
      return sendJSONResponse(res, HTTPStatus.INTERNAL_SERVER_ERROR, {
        success: false,
        message: err
