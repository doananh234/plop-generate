...
const mapDispatchToProps = (dispatch, props) => {
  return {
    ...
    updateRecord: (id, data) => dispatch(editRecord(props.resource, id, data)),
    gotoEditPage: id => dispatch(push(`${props.match.path}/${id}/edit`)),
    gotoShowPage: id => props.history.push(`${props.match.path}/${id}/show`),
    gotoCreatePage: () => props.history.push(`${props.match.path}/create`),
    deleteItem: id => dispatch(deleteRecord(props.resource, id)),
    pushQuery: filter => dispatch(push(`${props.match.path}?${getSearch(filter)}`)),
  };
};
